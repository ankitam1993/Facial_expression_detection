...
class Neural_network_raw_BN(object):
    ...
    def fit_predict(self, hidden_size_1, hidden_size_2):
        ...
        with graph.as_default():
            ...
            bn1 = self.batch_norm_wrapper(z1, is_training)
            hidden_layer_1 = tf.nn.relu(bn1)
            z2 = tf.add(tf.matmul(hidden_layer_1, self.w2), self.b2)
            bn2 = self.batch_norm_wrapper(z2, is_training)
            hidden_layer_2 = tf.nn.relu(bn2)
    ...
    def plot_training_loss(self):
        ...
        plt.savefig('Raw_pixes_training_loss_bn_%s.jpg' % (str(self.learning_rate)))
        plt.show()
